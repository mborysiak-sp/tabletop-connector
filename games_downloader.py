...
def extract_game_data(game_dict_raw):
    game_dict = {
        ...
        "name": game_dict_raw["name"][0]["@value"]
        if type(game_dict_raw["name"]) == list
...
def get_by_name(name):
    game_xml = requests.get(
        f"https://boardgamegeek.com/xmlapi2/search?query={name}&type=boardgame"
    )
...
def get_by_id(id):
    game_xml = requests.get(
        f"https://boardgamegeek.com/xmlapi2/thing?id={id}&type=boardgame"
    )
    game_dicts = xmltodict.parse(game_xml.content)
    return game_dicts["items"]["item"]
